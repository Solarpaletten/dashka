...
export function useEnglishTranslator() {
  ...
  // 🔥 v1.6 task3 — iOS Safari TTS unlock + voice selection
  ...
  const speakOriginal = useCallback((text: string, lang: string) => {
    ...
    // cancel any current speech first (iOS fix)
    ...
    // voice selection for better iOS compatibility
  ...
  const translate = useCallback(async (textOverride?: string) => {
    ...
    if (!text) return
    unlockTTS()  // 🔥 v1.6 task3 — unlock iOS TTS on user gesture
    ...
    if (SpeechRecognitionClass) {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        const isWrongLanguage = (text: string) => {
          ...
          if (sourceLang === 'ru-RU') {
            return /[a-zA-Z]/.test(text) // англ в русском режиме
          }

          if (sourceLang === 'en-US') {
            return /[а-яА-Я]/.test(text) // русский в английском режиме
          }
          
          if (isWrongLanguage(transcript)) {
            continue
          }
